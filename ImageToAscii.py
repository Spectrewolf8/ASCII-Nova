...
# def image_to_ascii_art(image, asciiRenderWidth=None, ASCII_CHARS=None):
#     if ASCII_CHARS is None:
#         ASCII_CHARS = ["@", "#", "$", "%", "?", "*", "+", ";", ":", ",", "."]  # Default ASCII characters
#     pixels = image.getdata()
#     print(pixels)
#     num_chars = len(ASCII_CHARS)
#     char_width = max(1, 256 // num_chars)  # Calculate the width of each ASCII character range
#     new_pixels = [ASCII_CHARS[pixel // char_width] for pixel in
#                   pixels]  # Replace each pixel with a character from the array
#     new_pixels = ''.join(new_pixels)
#     if asciiRenderWidth is None:
#         new_width = 120
#     else:
